...
from court_listener_sdk import CourtListener, AsyncCourtListener, DefaultAioHttpClient
from court_listener_sdk._utils import is_dict
...
logging.getLogger("court_listener_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[CourtListener]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with CourtListener(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncCourtListener]:
